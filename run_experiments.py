...
def plot_single_experiment_doa_accuracy(run_idx, test_type='static', folder_to_test_data=None, results_dir=None):
    ...
    fig.suptitle(f'Comprehensive Tracking Evaluation — Experiment {run_idx}', fontsize=16, fontweight='bold', y=0.96)

    # Panel 1: CSD True vs Estimate
    axs[0].plot(frames_x, true_csd, color='black', linewidth=2.5, label='True CSD', drawstyle='steps-mid')
    axs[0].plot(frames_x, est_csd + 0.05, color='darkviolet', linewidth=1.5, linestyle='--', label='Estimated CSD (+0.05 visual offset)', drawstyle='steps-mid')
    
    axs[0].set_yticks([0, 1, 2])
    axs[0].set_yticklabels(['0 (Noise)', '1 (Single)', '2 (Overlap)'])
    axs[0].set_ylabel('Active Speakers', fontweight='bold')
    ...
    # Save Layout
    single_plot_path = os.path.join(plot_dir, f'DOA_Analysis_Exp_{run_idx}.png')
    plt.savefig(single_plot_path, dpi=300, bbox_inches='tight')
    print(f"Saved 2-panel evaluation plot to: {single_plot_path}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise ValueError("Arguments parsed successfully.")
    except Exception as e:
