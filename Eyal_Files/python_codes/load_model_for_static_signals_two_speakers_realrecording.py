...
#        z_k_ilrma_second= z_k_ilrma_second[frame_before:index-frame_after]
    ...
    ################### source separation ###################################################
            ...
            if (Frame_classification_system[0,0]==0) & (Frame_classification_system[0,1]==0):
                ...
                s_hat = np.concatenate((s_hat.reshape(1,NUP),s_hat.reshape(1,NUP)*0.2))
...
# 
    ...
    ################################### save prediction to confusion matrix ######################
        ...
        speech2,t1=istft(s_hat_total[:,:,1].T, win, win, hop, nfft, fs)  
        speech1,t1=istft(s_hat_total[:,:,0].T, win, win, hop, nfft, fs)  

        speech1_clean,t1=istft(z_k_first[0,:,:], win, win, hop, nfft, fs)  
        speech2_clean,t1=istft(z_k_second[0,:,:], win, win, hop, nfft, fs)  

        scaled = speech/1.1/np.max(abs(speech))
        scaled1 = speech1/1.1/np.max(abs(speech1))
        scaled2 = speech2/1.1/np.max(abs(speech2))

        speech1_clean = speech1_clean/1.1/np.max(abs(speech1_clean))
        speech2_clean = speech2_clean/1.1/np.max(abs(speech2_clean))


        file_temp=(forlder_to_work2+'results/noisy_signal_%d.wav'%k)
        file_temp1=(forlder_to_work2+'results/separating_speaker_number_1_%d.wav'%k)
        file_temp2=(forlder_to_work2+'results/separating_speaker_number_2_%d.wav'%k)

        file_temp1_clean=(forlder_to_work2+'results/clean_signal_1_%d.wav'%k)
        file_temp2_clean=(forlder_to_work2+'results/clean_signal_2_%d.wav'%k)    

        write(file_temp, fs,scaled)
        write(file_temp1, fs,scaled1)
        write(file_temp2, fs,scaled2)

        write(file_temp1_clean, fs,speech1_clean)
        write(file_temp2_clean, fs,speech2_clean)
...
plt.show()
plt.savefig(folder_to_save+'fig1.png')
...
ax2.xaxis.set_major_formatter(ticks_x)
plt.show()
plt.savefig(folder_to_save+'fig2.png')
...
plt.show()
plt.savefig(folder_to_save+'fig3.png')
...
axes.set_ylim([-1.5,1.5])
plt.show()
plt.savefig(folder_to_save+'fig4.png')
