...
folder_names = ['0','5','10'] #folder_names = ['300','350','400','450','500','550']

folder_to_save = 'C:/project/static_signals/two_speakers/SIR/'
...
if new:
    ...
    sir_ilerma = np.zeros((len(folder_names),nom_data_sets-1,2))
    ...
    sir_ilerma=np.load(folder_to_save+'sir_ilerma.npy')
...
for d in range(0,len(folder_names)):
    forlder_to_work2 = ('C:/project/static_signals/two_speakers/SIR/SNR_20_T60_300_SIR_%s/'%folder_names[d])
    ...
    ################################### bss eval ################################
        ...
        (sdr_noise,sir_noise,sar_noise,perm)  = mir_eval.separation.bss_eval_sources(reference_sources.T+1e-9, noisy_sources.T, compute_permutation=True)
    ...
    ###################################### alg results ##################################################
        ...
        first_stoi = stoi(z_k_first_one_speaker_time,s_hat_first_one_speaker_time,fs)
        first_stoi_noisy = stoi(z_k_first_one_speaker_time,z_k_noise_time,fs)
        
        first_pesq = pesq(fs,z_k_first_one_speaker_time,s_hat_first_one_speaker_time,'wb')
   ...
   ###################################### ilrma results ##################################################  
        (sdr,sir,sar,perm)  = mir_eval.separation.bss_eval_sources(reference_sources.T+1e-9, estimated_sources_ilerma.T, compute_permutation=True)
        
        sir_ilerma[d,k-1,:] = sir
        ...
        stoi_ilerma[d,k-1] = first_stoi_ilerma
        pesq_ilerma[d,k-1] = first_pesq_ilerma
    ...
    ################################### save prediction to confusion matrix ######################
        ...
        np.save(folder_to_save+'sir_ilerma.npy', sir_ilerma)
        ...
        for p in range(0,num_speech):
            speech,t1=istft(s_hat_total_for_test[:,:,p].T, win, win, hop, nfft, fs)  
            # scaled = speech/np.max(speech)
            file_temp=(forlder_to_work2+'results/signals/separating_speaker_number_{}_{}.wav'.format(p,k))
            write(file_temp, fs,speech)
    
        speech,t1=istft(z_k_noisy[:,:,0].T, win, win, hop, nfft, fs)  
        file_temp=(forlder_to_work2+'results/signals/noisy_signal_%d.wav'%k)
        write(file_temp, fs,speech)
        
        speech,t1=istft(z_k_first_one_speaker[:,:,0].T, win, win, hop, nfft, fs)  
        file_temp=(forlder_to_work2+'results/signals/first_clean_%d.wav'%k)
        write(file_temp, fs,speech)
        
        speech,t1=istft(z_k_second_one_speaker[:,:,0].T, win, win, hop, nfft, fs)  
        file_temp=(forlder_to_work2+'results/signals/second_clean_%d.wav'%k)
        write(file_temp, fs,speech)
        
        speech,t1=istft(z_k_noise_one_speaker[:,:,0].T, win, win, hop, nfft, fs)  
        file_temp=(forlder_to_work2+'results/signals/only_noise_%d.wav'%k)
        write(file_temp, fs,speech)  
        

        file_temp=(forlder_to_work2+'results/signals/ilerma_first_%d.wav'%k)
        write(file_temp, fs,s_hat_first_one_speaker_ilerma_time)
        

        file_temp=(forlder_to_work2+'results/signals/ilerma_second_%d.wav'%k)
        write(file_temp, fs,s_hat_second_one_speaker_ilerma_time) 
...
line_3, = plt.plot(stoi_ilerma.mean(axis=1), label='ILRMA')
...
line_3, = plt.plot(pesq_ilerma.mean(axis=1), label='ILRMA')
plt.legend(handles=[line_1,line_2,line_3])
plt.xlabel('SIR')
plt.ylabel('Score')
xi = list(range(len(stoi_alg)))
...
line_3, = plt.plot((sir_ilerma[:,:,0]-sir_noisy[:,:,0]).mean(axis=1), label='ILRMA')
