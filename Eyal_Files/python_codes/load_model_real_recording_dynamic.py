...
if new:
    stoi_alg = np.zeros((nom_data_sets-1))
    stoi_noisy = np.zeros((nom_data_sets-1))
  
    pesq_alg = np.zeros((nom_data_sets-1))
    pesq_noisy = np.zeros((nom_data_sets-1))

    sir_noisy = np.zeros((nom_data_sets-1))
    sir_alg = np.zeros((nom_data_sets-1))
    
    sdr_noisy = np.zeros((nom_data_sets-1))
    sdr_alg = np.zeros((nom_data_sets-1))
    ...
    sir_alg=np.load(folder_to_save+'sdr_alg.npy')
...
#                                Qyy_second = Qyy_stand
                            ...
                            if (min1>(threshold_chage_location-2)) & (min2>(threshold_chage_location-2)):
                                if (time_first-min1*30)>(time_second-min2*30):
                                    ...
                                    second_speaker_active = 0.1
                                    to_change = 0
                                else: 
                                    first_speaker_active = 0.1
...
################################### bss eval ################################
#    
#        s_hat_first = np.zeros((len(x_test),NUP,2),complex) 
#        s_hat_second = np.zeros((len(x_test),NUP,2),complex)
#        s_hat_noise = np.zeros((len(x_test),NUP,2),complex)
#        
#        for l in range(len(x_test)):
#            for j in range(NUP):
#                W_temp = W[l,j,:,:].conj().T
#                s_hat_first[l,j,:]=W_temp@z_k_first[:,j,l]
#                s_hat_second[l,j,:]=W_temp@z_k_second[:,j,l]
#                s_hat_noise[l,j,:]=W_temp@z_k_noise[:,j,l]
#                
#        
#        
#        vad1= vad1[frame_before:index-frame_after]
#        vad2= vad2[frame_before:index-frame_after]
#        
#        
#        vad1_one_speaker = vad1[start_lcmv:]
#        vad2_one_speaker = vad2[start_lcmv:]
#        s_hat_first_one_speaker=s_hat_first[start_lcmv:]
#        s_hat_second_one_speaker=s_hat_second[start_lcmv:]
#        s_hat_noise=s_hat_noise[start_lcmv:]
#        z_k_first_one_speaker=z_k_first.T[start_lcmv:]
#        z_k_second_one_speaker=z_k_second.T[start_lcmv:]
#        z_k_noise=z_k_noise.T[start_lcmv:]
#        z_k_noisy=z_k[start_lcmv:]
#        
#        z_k_noisy_first=z_k_noisy[np.where(vad1_one_speaker==1)]
#        z_k_noisy_second=z_k_noisy[np.where(vad2_one_speaker==1)]
#        
#        s_hat_first_one_speaker=s_hat_first_one_speaker[np.where(vad1_one_speaker==1)]
#        s_hat_second_one_speaker=s_hat_second_one_speaker[np.where(vad2_one_speaker==1)]
#        z_k_first_one_speaker=z_k_first_one_speaker[np.where(vad1_one_speaker==1)]
#        z_k_second_one_speaker=z_k_second_one_speaker[np.where(vad2_one_speaker==1)]    
#        
#        s_hat_first_one_speaker_time0,_=istft(s_hat_first_one_speaker[:,:,0].T, win, win, hop, nfft, fs)
#        s_hat_first_one_speaker_time1,_=istft(s_hat_first_one_speaker[:,:,1].T, win, win, hop, nfft, fs)
#        s_hat_second_one_speaker_time0,_=istft(s_hat_second_one_speaker[:,:,0].T, win, win, hop, nfft, fs)
#        s_hat_second_one_speaker_time1,_=istft(s_hat_second_one_speaker[:,:,1].T, win, win, hop, nfft, fs)
#        s_hat_noise_time0,_=istft(s_hat_noise[:,:,0].T, win, win, hop, nfft, fs)
#        s_hat_noise_time1,_=istft(s_hat_noise[:,:,1].T, win, win, hop, nfft, fs)
#        s_hat_noise_time=(s_hat_noise_time0+s_hat_noise_time1)/2
...
#        z_k_noise_time,_=istft(z_k_noise[:,:,0].T, win, win, hop, nfft, fs)
#        
#        z_k_first_noisy_time,_=istft(z_k_noisy_first[:,:,0].T, win, win, hop, nfft, fs)
#        z_k_second_noisy_time,_=istft(z_k_noisy_second[:,:,0].T, win, win, hop, nfft, fs)    
#        
#        first_separation = max(z_k_first_one_speaker_time.std()/s_hat_first_one_speaker_time0.std(),z_k_first_one_speaker_time.std()/s_hat_first_one_speaker_time1.std())
#        first_separation = 10*np.log10((first_separation)**2)
#        
#        second_separation = max(z_k_second_one_speaker_time.std()/s_hat_second_one_speaker_time0.std(),z_k_second_one_speaker_time.std()/s_hat_second_one_speaker_time1.std())
#        second_separation = 10*np.log10((second_separation)**2)
#        
#        noise_separation = 10*np.log10((z_k_noise_time.std()/s_hat_noise_time.std())**2)
#        
#        first_stoi = max(stoi(z_k_first_one_speaker_time,s_hat_first_one_speaker_time0,fs),stoi(z_k_first_one_speaker_time,s_hat_first_one_speaker_time1,fs))
#        second_stoi = max(stoi(z_k_second_one_speaker_time,s_hat_second_one_speaker_time0,fs),stoi(z_k_second_one_speaker_time,s_hat_second_one_speaker_time1,fs))
#    
#        first_stoi_noisy = stoi(z_k_first_one_speaker_time,z_k_first_noisy_time,fs)
#        second_stoi_noisy = stoi(z_k_second_one_speaker_time,z_k_second_noisy_time,fs)
#    
#    
#        first_pesq = max(pesq(fs,z_k_first_one_speaker_time,s_hat_first_one_speaker_time0,'wb'),pesq(fs,z_k_first_one_speaker_time,s_hat_first_one_speaker_time1,'wb'))
#        second_pesq = max(pesq(fs,z_k_second_one_speaker_time,s_hat_second_one_speaker_time0,'wb'),pesq(fs,z_k_second_one_speaker_time,s_hat_second_one_speaker_time1,'wb'))
#    
#        first_pesq_noisy = pesq(fs,z_k_first_one_speaker_time,z_k_first_noisy_time,'wb')
#        second_pesq_noisy = pesq(fs,z_k_second_one_speaker_time,z_k_second_noisy_time,'wb') 
#        
#        separation = (first_separation+second_separation)/2
#        separation_total = np.append(separation_total,separation)
#        noise_separation_total = np.append(noise_separation_total,noise_separation)
#        
#        stoi_temp = (first_stoi+second_stoi)/2
#        stoi_total = np.append(stoi_total,stoi_temp)
#        
#        pesq_temp = (first_pesq+second_pesq)/2
#        pesq_total = np.append(pesq_total,pesq_temp)
#        
#        
#        stoi_temp_noisy = (first_stoi_noisy+second_stoi_noisy)/2
#        stoi_total_noisy = np.append(stoi_total_noisy,stoi_temp_noisy)
#        
#        pesq_temp_noisy = (first_pesq_noisy+second_pesq_noisy)/2
#        pesq_total_noisy = np.append(pesq_total_noisy,pesq_temp_noisy)
...
#        s_hat_total_for_test = start_lcmv_overlap[start_lcmv:]
#        z_k_first_one_speaker=z_k_first_overlap[start_lcmv:]
#        z_k_second_one_speaker=z_k_second_overlap[start_lcmv:]
#        z_k_noisy=z_k_overlap[start_lcmv:]
    ...
    sir_noisy[k-1] = sir_noise.mean()
    sdr_noisy[k-1] = sdr_noise.mean()
    ...
    sir_alg[k-1] = sir.mean()
    sdr_alg[k-1] = sdr.mean()
...
#        separation_total = np.append(separation_total,separation)
#    
    stoi_alg[k-1] = (first_stoi+second_stoi)/2
    pesq_alg[k-1] = (first_pesq+second_pesq)/2   
    stoi_noisy[k-1] = (first_stoi_noisy+second_stoi_noisy)/2
    pesq_noisy[k-1]  = (first_pesq_noisy+second_pesq_noisy)/2
