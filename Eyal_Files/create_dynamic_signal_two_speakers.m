...
%% create speakers clean
    ...
    for g=1:8
        file1=f1(g+2).name;
        file2=f2(g+2).name;
        source1 = strcat(folder1,'\',file1);
        source2 = strcat(folder2,'\',file2);
        speech_1_temp = audioread(source1);
        speech_2_temp = audioread(source2);
        speech_1=speech_1_temp(:,1);
        speech_2=speech_2_temp(:,1);
        overlap_value = min(floor(length(speech_1)*overlap),floor(length(speech_2)*overlap));
        if mod(g,2)
            pad_zeros1=zeros(1,fs/2*randi([1,3]))';
            in1 = [in1 ; pad_zeros1 ; speech_1];
            vad1 = [vad1 ; pad_zeros1 ; ones(1,length(speech_1))'];
            pad_zeros2=zeros(1,length(in1)-length(in2)-overlap_value)';
            in2 = [in2 ; pad_zeros2 ; speech_2];
            vad2 = [vad2 ; pad_zeros2 ; ones(1,length(speech_2))'];
        else
            pad_zeros2=zeros(1,fs/2*randi([1,3]))';
            in2 = [in2 ; pad_zeros2 ; speech_2]; 
            vad2 = [vad2 ; pad_zeros2 ; ones(1,length(speech_2))'];
            pad_zeros1=zeros(1,length(in2)-length(in1)-overlap_value)';
            in1 = [in1 ; pad_zeros1 ; speech_1];
            vad1 = [vad1 ; pad_zeros1 ; ones(1,length(speech_1))'];
        end
...
%% signal generator     
    ...
    for ii = 1:hop:len
        ...
        if mean(vad(ii:1:min(ii+hop-1,len)))<1.5 
            ii_index = ii_index+foward_index;
        end
        
        if mod(ii_index,140)==0
    ...
    [receiver_first,beta_hat] = signal_generator(in1,c,fs,rp_path,sp_path_first,L,beta,n,'o',order);
...
%% calc speakers STD and norm  
    ...
    receiver_first = receiver_first/A_x_first;
    receiver_second = receiver_second/A_x_second;   
...
%% create input
    ...
    for j=20:2.5:20
        ...
        receivers=(receivers+mic_noise+A_n_diffuse*difuse_noise.'+A_n_diraction*Receivers_noise).'; 
        noise_only = (mic_noise+A_n_diffuse*difuse_noise.'+A_n_diraction*Receivers_noise).';
        receivers=receivers/max(max(abs(receivers)));
        noise_only=noise_only/max(max(abs(noise_only)));        

        value = num2str(j);
        path = sprintf('C:\\project\\dynamic_signals\\two_speakers\\T60\\SNR_%s_T60_350_SIR_0_overlap_50\\',value);
    ...
    for j=0:5:0
        ...
        Ax_SIR = A_x_before/(10^(j/20));
        receivers = Ax_SIR*receiver_first.'+receiver_second.';
        %clac An
        A_x=mean(std(receivers'));
        std_n=mean(std(Receivers_noise'));
        A_n_diffuse = A_x/(10^(SNR_difuse/20));
        A_n_diraction = A_x/(10^(SNR_direction/20));
        A_n_mic = A_x/(10^(SNR_mic/20));
        ...
        value = num2str(j);
        path = sprintf('C:\\project\\dynamic_signals\\two_speakers\\T60\\SNR_20_T60_350_SIR_%s_overlap_50\\',value);
        data_together=strcat(path,sprintf('dynamic_signal_%d.wav',i));
        data_first=strcat(path,sprintf('dynamic_signal_first_%d.wav',i));
        data_second=strcat(path,sprintf('dynamic_signal_second_%d.wav',i));
        data_noise=strcat(path,sprintf('dynamic_signal_noise_%d.wav',i));
