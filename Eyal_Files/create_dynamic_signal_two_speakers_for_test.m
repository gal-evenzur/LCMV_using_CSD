...
%% create speakers clean
    ...
    source15 = strcat(folder1,'\',file15);
    source16 = strcat(folder1,'\',file16);
    
    source21 = strcat(folder2,'\',file21);
    source22 = strcat(folder2,'\',file22);
    source23 = strcat(folder2,'\',file23);
    source24 = strcat(folder2,'\',file24);
    source25 = strcat(folder2,'\',file25);
    source26 = strcat(folder2,'\',file26);
    ...
    speech_15_temp = audioread(source15);
    speech_16_temp = audioread(source16);
    ...
    speech_25_temp = audioread(source25);
    speech_26_temp = audioread(source26);

    speech_11=speech_11_temp(:,1);
    speech_12=speech_12_temp(:,1);
    speech_13=speech_13_temp(:,1);
    speech_14=speech_14_temp(:,1);
    speech_15=speech_15_temp(:,1);
    speech_16=speech_16_temp(:,1);
    
    speech_21=speech_21_temp(:,1);
    speech_22=speech_22_temp(:,1);
    speech_23=speech_23_temp(:,1);
    speech_24=speech_24_temp(:,1);
    speech_25=speech_25_temp(:,1);
    speech_26=speech_26_temp(:,1);

    pad_zeros1=zeros(1,fs)';
    pad_zeros2=zeros(1,fs*2)';
    
    firtst_alone = [speech_11 ;speech_12 ;speech_13 ; speech_14];
    firtst_together = [speech_15 ;speech_16];
    second_alone = [speech_21 ;speech_22 ;speech_23 ; speech_24];
    second_together = [speech_25 ;speech_26];
...
%% signal generator     
    ...
    for ii = 1:hop:len
        ...
        if mean(vad(ii:1:min(ii+hop-1,len)))<1.5 && mean(vad1(ii:1:min(ii+hop-1,len)))>0.5
    ...
    for ii = 1:hop:len
        ...
        if mean(vad(ii:1:min(ii+hop-1,len)))<1.5 && mean(vad2(ii:1:min(ii+hop-1,len)))>0.5
            ii_index = ii_index+foward_index;
        end
...
%% create input
    ...
    for j=10:2.5:20
        ...
        value = num2str(j);
        path = sprintf('C:\\project\\dynamic_signals\\two_speakers_long\\SNR\\SNR_%s_T60_300_SIR_0\\',value);
    ...
    for j=0:5:15
        ...
        value = num2str(j);
        path = sprintf('C:\\project\\dynamic_signals\\two_speakers_long\\SIR\\SNR_20_T60_300_SIR_%s\\',value);
        data_together=strcat(path,sprintf('dynamic_signal_%d.wav',i));
        data_first=strcat(path,sprintf('dynamic_signal_first_%d.wav',i));
        data_second=strcat(path,sprintf('dynamic_signal_second_%d.wav',i));
        data_noise=strcat(path,sprintf('dynamic_signal_noise_%d.wav',i));
