...
mixSig = audioread('./input/dynamic_signal_1.wav');
...
for i = 1:8
    [estSig, cost] = ILRMAISS(mixSig(1+(i-1)*2*resampFreq:1+i*2*resampFreq-1,:), nSrc, resampFreq, nBases, fftSize, shiftSize, windowType, nIter, ilrmaType, refMic, applyNormalize, applyWhitening, drawConv);
    estSig_total = [estSig_total ; estSig];
end
...
audiowrite(sprintf('%s/estimatedSignal_dynamic1.wav', outputDir), estSig_total(:,1), resampFreq); % estimated signal 1
audiowrite(sprintf('%s/estimatedSignal_dynamic2.wav', outputDir), estSig_total(:,2), resampFreq); % estimated signal 2
